...
def evaluation(weather):
    ...
    with open('apollo_record/wk_record/S8/Apollo_trace.pickle', 'rb') as file:
        output_Apollo_trace = pickle.load(file)
    file.close()
    with open('apollo_record/wk_record/S8/my_trace.pickle', 'rb') as file:
      output_my_trace1 = pickle.load(file)
    file.close()
    with open('apollo_record/wk_record/S8/LLM_trace_CH.pickle', 'rb') as file:
    ...
    for spec_index in range(len(specifications_in_scenario)):
            ...
            _lawbreak(output_my_trace1, single_specification, weather)
...
if __name__ == "__main__":
    weather = {'rain' : 1.0,  'snow': 0.1, 'fog' : 0.0}
    evaluation(weather)