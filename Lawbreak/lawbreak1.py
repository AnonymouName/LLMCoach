...
class Mutation:
    ...
    def writer(self, num):
        for data in self.testcase:
            try:
                os.remove(self.original_data_['id']+'.json')
            except:
                pass
            with open(self.original_data_['id']+'.json' , "w") as json_file:
                json.dump(data, json_file)
            self.name = self.original_data_['id'] + '_' + str(num) + '_' + str(self.num)
            with open('record/' + self.original_data_['id'] + '_' + str(num) + '_' + str(self.num) +'.json' , "w") as json_file:
                json.dump(data, json_file)
                self.num += 1
    ...
    def run(self):
        ...
        name.clear()
        name.send_keys(self.original_data_['id'] + '_' + str(self.times_num) + '_' + str(self.num))
    ...
    def save_message(self, path_name, file_name, item, step = 1):
        if item not in self.record:
            self.record[item] = {}
        record = Record(path_name + file_name)
        time_list = sorted([t for topic, message, t in record.read_messages(_map[item])])
        # if step != 1:
        #     time_list = time_list[::step]
        # elif item in ['obstacles', 'traffic_light']:
    ...
    def check_record(self, path, save_path):
        # cmd = "rm -rf apollo_record/*"
        # subprocess.run(cmd, shell=True, capture_output=True, text=True)
        # cmd = "docker cp apollo_neo_dev_core:/home/sunsun/.apollo/resources/records/ apollo_record/"
        # subprocess.run(cmd, shell=True, capture_output=True, text=True)
        files = os.listdir(path)
        sorted_files = []
        for file_name in files:
            if '_s' not in file_name:
        ...
        for file_name in sorted_files:
            self.save_message(path, file_name, 'pose')
            key_list = list(_map.keys())
            key_list.remove('pose')
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
               executor.map(self.save_message, [path] * (len( _map.keys()) - 1), [file_name] * (len( _map.keys()) - 1), key_list)
...
if __name__ == "__main__":
    ...
    # name_list = ["S8"]
    s_path = "LLM_record/S"
    record = os.listdir(s_path)
    map_name = 'sunnyvale'
    # with open(dir+name, 'r') as js_file:
    ...
    for file_name in record:
        with open(s_path + "/" + file_name + "/" + file_name + ".json", 'r') as js_file:
            data = json.load(js_file)
        map_Dir = data['scenario']['mapDir']
        print(file_name)
        for name in [item for item in os.listdir(s_path+"/"+file_name) if os.path.isdir(os.path.join(s_path+"/"+file_name, item))]:
            map_info = get_map_info(map_name)
            mutation = Mutation(data, map_name, map_info, "65e0308b6f0ae871927b3bce")
            # mutation.testcase_random(num=1)
            # mutation.writer(times)
            # path = "apollo_record/wk_record/"+ name +"/LLM/CH/"
            # save_path = 'apollo_record/wk_record/' + name + '/LLM_trace_CH.pickle'
            path = s_path + "/" + file_name + "/" + name +"/"
            save_path = s_path + "/" + file_name + "/" + name +"_trace.pickle"
            mutation.check_record(path, save_path)
        # mutation.lawbreak()
